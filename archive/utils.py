...
def read_process_text(filename="tiny-shakespeare.txt", subset=None):
    ...
    chars = set([x.strip() for x in "".join(text).replace("\n", " \n ").split(" ")]).union(["<"]).union(set("".join(text)))
...
class textDatasetFromFile(Dataset):
    ...
    def __getitem__(self, index):
        return torch.from_numpy(self.input_seq[index]), torch.Tensor(self.target_seq[index])
